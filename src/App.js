...
class App extends Component {
  ...
  createFlowChart() {
    ...
    for (var e in json.items) {
      if (json.items.hasOwnProperty(e)) {
        items.push(json.items[e]);
      }
    }
    this.span_into_grid(json);
    this.elements_locations(grids);
    this.draw_streams(json);
    this.draw_objects(json);
    this.link_objects(json);
  }
    ...
    for (var a = "", s = "", n = "", i = "", o = 0; o < e.length; o++) {
      for (var d = 0; d < r.length; d++)
          for (var l = 0; l < r[d].length; l++) {
            ...
            // r[d][l] == t ? (a = d, n = l) : r[d][l] == e[o] && (s = d, i = l);
          }
      if (a != s && n > i)
  ...
  draw_objects(t) {
    var e = "",
        r = [],
        a = d3.select("#flowChart_div").select("svg").selectAll("g").data(items);
    ...
    var text = a.append("foreignObject").append("xhtml:body").html(function(t) {
        ...
        var i = d3.select("svg").selectAll("g.link_from_" + items[r].id).data(s);
          i.enter()
            ...
            .attr("d", function(t, r) {
                ...
                var n = "M" + (Number(e[a].x) + Number(e[a].width) / 2) + "," + (Number(e[a].y) + Number(e[a].height)),
  ...
  render() {
    return (
      <div className="App">
        <div id="flowChart_div" className="flowChart_div">
        </div>
      </div>
    );
  }
